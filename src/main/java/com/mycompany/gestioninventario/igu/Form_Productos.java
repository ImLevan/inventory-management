...
public class Form_Productos extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        lbTitle.setText("Lista de productos");
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Categoria", "Nombre", "Marca", "Detalles"
            }
        ) {
    ...
    public void cargarDatos() {
        ...
        if (listaProductos != null) {
            for (Producto producto : listaProductos) {
                Object[] objeto = {
                        producto.getId(),                      
                        producto.getProducto_categoria().getNombre(),
                        producto.getNombre(),
                        producto.getMarca(),
    ...
    private void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarActionPerformed
        CargarProducto pantallaProducto = new CargarProducto();
        pantallaProducto.setFormProductosReference(this);
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        if(table.getRowCount() > 0){
            if(table.getSelectedRow() != -1){
                int num_producto= Integer.parseInt (String.valueOf(table.getValueAt(table.getSelectedRow(), 0)));

                ModificarProducto pantallaModif = new ModificarProducto(num_producto);
                pantallaModif.setForm_ProductoReference(this);
                pantallaModif.setVisible(true);
                pantallaModif.setLocationRelativeTo(null);

            }
            else{
                MostrarMensaje.mostrarMensaje("No se seleccionó ningun producto", "Error", "Error al editar");
            }
        }
        else{
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if(table.getRowCount() > 0){
            if(table.getSelectedRow() != -1){
                int num_producto = Integer.parseInt (String.valueOf(table.getValueAt(table.getSelectedRow(), 0)));
                control.borrarProducto(num_producto);

                MostrarMensaje.mostrarMensaje("Producto eliminado con exito", "Info", "Borrado de producto");
                cargarDatos();
            }
            else{
                MostrarMensaje.mostrarMensaje("No se seleccionó ningun producto", "Error", "Error al eliminar");
            }
        }
        else{
